...
def fillObsCPT(bayesNet, gameState):
    ...
    "*** YOUR CODE HERE ***"
    #print gameState.getPossibleHouses() [(3, 3), (3, 7), (11, 3), (11, 7)]
    #create a dictionary for the possible house position
    #TOP_LEFT_VAL = "topLeft"
    #TOP_RIGHT_VAL = "topRight"
    #BOTTOM_LEFT_VAL = "bottomLeft"
    #BOTTOM_RIGHT_VAL = "bottomRight"
    #HOUSE_VALS = [TOP_LEFT_VAL, TOP_RIGHT_VAL, BOTTOM_LEFT_VAL, BOTTOM_RIGHT_VAL]
    house_position = {}
    house_position[bottomLeftPos] = BOTTOM_LEFT_VAL
    house_position[topLeftPos] = TOP_LEFT_VAL
    house_position[bottomRightPos] = BOTTOM_RIGHT_VAL
    house_position[topRightPos] = TOP_RIGHT_VAL
    #now we need traverse all the house position
    for house_p in gameState.getPossibleHouses() :
        for observation_p in gameState.getHouseWalls(house_p) :
            #get the observation variable
            observation_var = OBS_VAR_TEMPLATE % observation_p
            #create a factor for the current observation var
            observation_factor = bn.Factor([observation_var],[FOOD_HOUSE_VAR,GHOST_HOUSE_VAR],bayesNet.variableDomainsDict())
            #give factor possibility
            for assignment in observation_factor.getAllPossibleAssignmentDicts() :
                wall_color = assignment[observation_var]
                #print wall_color
                #probablity for the food house
                if assignment[FOOD_HOUSE_VAR] == house_position[house_p] :
                    if wall_color == RED_OBS_VAL : #the food has the red wall
                        assignment_probability = PROB_FOOD_RED
                    elif wall_color == BLUE_OBS_VAL : #the food has the blue wall
                        assignment_probability = 1 - PROB_FOOD_RED
                    else :
                        assignment_probability = 0
                #probability for the ghost house
                elif assignment[GHOST_HOUSE_VAR] == house_position[house_p] :
                    if wall_color == RED_OBS_VAL : #the ghost has the red wall
                        assignment_probability = PROB_GHOST_RED
                    elif wall_color == BLUE_OBS_VAL : #the ghost has the blue wall
                        assignment_probability = 1 - PROB_GHOST_RED
                    else :
                        assignment_probability = 0
                #probability for the neither food nor ghost
                else :
                    if wall_color == NO_OBS_VAL : 
                        assignment_probability = 1
                    else :
                        assignment_probability = 0
                #set the probabilty for the current observation factor
                observation_factor.setProbability(assignment,assignment_probability)
            bayesNet.setCPT(observation_var,observation_factor)
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    probability = -float('inf') #initial to the smallest probability
    #traverse all the assignment to get the most likely assignment
    for assignment in assignments:
        tmp_probability = factor.getProbability(assignment)
        if tmp_probability > probability :
            probability = tmp_probability
            most_likely_assignment = assignment
        else :
            continue
    return most_likely_assignment
    #util.raiseNotDefined()
