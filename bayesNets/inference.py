...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        "*** YOUR CODE HERE ***"
        #joinFactorsByVariable:def joinFactorsByVariable(factors, joinVariable):
        #input factors is a list of factors. Input joinVariable is the variable to join on.
        #Returns a tuple of (factors not joined, resulting factor from joinFactors)
        #first,grab all factors where we know the evidence variables (to reduce the size of the tables)
        currentFactorsList = bayesNet.getAllCPTsWithEvidence(evidenceDict)
        #eliminate all factors by the order of the eliminationOrder
        for elimination_var in eliminationOrder:
            currentFactorsList, joinedFactor = joinFactorsByVariable(currentFactorsList, elimination_var)
            #here we consider those have more than 1 unconditioned vars
            if len(joinedFactor.unconditionedVariables()) > 1:#if a factor that you are about to eliminate a variable from has only one unconditioned variable, you should not eliminate it 
                #now we need to eliminate this joinedFactor
                after_elimination_factor = eliminate(joinedFactor,elimination_var)
                #append this new factor to the currentFactorsList
                currentFactorsList.append(after_elimination_factor)
        new_factors = joinFactors(currentFactorsList)
        return normalize(new_factors)
